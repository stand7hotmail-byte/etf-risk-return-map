...
def test_portfolio_volatility(mock_data):
    ...
    # w.T * C * w = 
    # [1/3, 1/3, 1/3] * [[0.10, 0.02, 0.01], [0.02, 0.15, 0.03], [0.01, 0.03, 0.12]] * [1/3, 1/3, 1/3]^T
...
def test_portfolio_sharpe_ratio(mock_data):
    ...
    sharpe_ratio = portfolio_sharpe_ratio(mock_data['weights'], mock_data['avg_returns'], mock_data['cov_matrix'], RISK_FREE_RATE)
...
def test_portfolio_sharpe_ratio_zero_volatility(mock_data):
    ...
    sharpe_ratio = portfolio_sharpe_ratio(mock_data['weights'], mock_data['avg_returns'], zero_cov_matrix, RISK_FREE_RATE)
